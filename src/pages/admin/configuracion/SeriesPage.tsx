...
const SeriesPage: React.FC = () => {
  ...
  useEffect(() => {
    cargarSeries();
  }, [filtroTipoDoc, soloActivas]);

  const cargarSeries = async () => {
    setLoading(true);
    setError('');
    try {
      const data = await listarSeries(filtroTipoDoc || undefined, soloActivas);
      setSeries(data);
    } catch (err: any) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (editandoSerie) {
        // Actualizar: solo descripcion, activa, sucursalId
        await actualizarSerie(editandoSerie.id!, {
  ...
  const handleEliminar = async () => {
    ...
    try {
      await eliminarSerie(serieSeleccionada.id!);
  ...
  const handleReiniciarCorrelativo = async () => {
    ...
    try {
      await reiniciarCorrelativo(serieSeleccionada.id!);
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">
            No hay series registradas. Crea una nueva serie para comenzar.
          </div>
        ) : (
            ...
            <>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Puedes modificar la serie si necesitas otro código
                </p>
              </div>
            </>
          )}
          ...
          {editandoSerie && (
            <div className="bg-gray-50 p-3 rounded">
              <p className="text-sm text-gray-700">
                <span className="font-medium">Tipo:</span> {editandoSerie.tipoDoc} -{' '}
                {TIPOS_DOCUMENTO[editandoSerie.tipoDoc]}
              </p>
              <p className="text-sm text-gray-700">
